...
CREATE TABLE wiki_user (
  ...
  wiki_user_name                 VARCHAR(255) NOT NULL UNIQUE,
  wiki_user_real_name            VARCHAR(255),
  wiki_user_password             VARCHAR(1024),
  wiki_user_newpassword          VARCHAR(1024),
  wiki_user_newpass_time         TIMESTAMP(3),
  wiki_user_token                VARCHAR(255),
  wiki_user_email                VARCHAR(1024),
  wiki_user_email_token          VARCHAR(255),
  wiki_user_email_token_expires  TIMESTAMP(3),
  wiki_user_email_authenticated  TIMESTAMP(3),
  -- obsolete, replace by wiki_user_properties table
  -- wiki_user_options              CLOB(64K) INLINE LENGTH 4096,
  wiki_user_touched              TIMESTAMP(3),
  wiki_user_registration         TIMESTAMP(3),
  wiki_user_editcount            INTEGER
);
CREATE INDEX wiki_user_email_token_idx
  ON wiki_user (wiki_user_email_token);
CREATE UNIQUE INDEX wiki_user_include_idx
  ON wiki_user (wiki_user_id)
  INCLUDE (wiki_user_name, wiki_user_real_name, wiki_user_password, wiki_user_newpassword,
    wiki_user_newpass_time, wiki_user_token,
    wiki_user_email, wiki_user_email_token, wiki_user_email_token_expires,
    wiki_user_email_authenticated,
    wiki_user_touched, wiki_user_registration, wiki_user_editcount);
CREATE UNIQUE INDEX wiki_user_email
  ON wiki_user (wiki_user_email);



-- Create a dummy wiki_user to satisfy fk contraints especially with revisions
INSERT INTO wiki_user(
  wiki_user_name,    wiki_user_real_name,           wiki_user_password,  wiki_user_newpassword,   wiki_user_newpass_time,
  wiki_user_email,   wiki_user_email_authenticated, wiki_user_token,     wiki_user_registration,  wiki_user_editcount
)
VALUES (
...
CREATE TABLE wiki_user_groups (
  ug_wiki_user   BIGINT NOT NULL DEFAULT 0,
  --    REFERENCES wiki_user(wiki_user_id) ON DELETE CASCADE,
  ug_group  VARCHAR(255) NOT NULL
);
CREATE INDEX wiki_user_groups_unique
  ON wiki_user_groups (ug_wiki_user, ug_group);



CREATE TABLE wiki_user_newtalk (
  -- registered wiki_users key
  wiki_user_id              BIGINT NOT NULL DEFAULT 0,
  --  REFERENCES wiki_user(wiki_user_id) ON DELETE CASCADE,
  -- anonymous wiki_users key
  wiki_user_ip              VARCHAR(40),
  wiki_user_last_timestamp  TIMESTAMP(3)
);
CREATE INDEX wiki_user_newtalk_id_idx
  ON wiki_user_newtalk (wiki_user_id);
CREATE INDEX wiki_user_newtalk_ip_idx
  ON wiki_user_newtalk (wiki_user_ip);
CREATE UNIQUE INDEX wiki_user_newtalk_include_idx
  ON wiki_user_newtalk (wiki_user_id, wiki_user_ip)
  INCLUDE (wiki_user_last_timestamp);
...
CREATE TABLE revision (
  ...
  rev_wiki_user          BIGINT NOT NULL DEFAULT 0,
  --  REFERENCES wiki_user(wiki_user_id) ON DELETE RESTRICT,
  rev_wiki_user_text     VARCHAR(255) NOT NULL,
...
CREATE INDEX rev_wiki_user_idx
  ON revision (rev_wiki_user);
CREATE INDEX rev_wiki_user_text_idx
  ON revision (rev_wiki_user_text);
...
CREATE INDEX archive_wiki_user_text
  ON archive (ar_wiki_user_text);
...
CREATE UNIQUE INDEX eu_external_id_idx
  ON external_wiki_user (eu_external_id)
  INCLUDE (eu_local_id);
CREATE UNIQUE INDEX eu_local_id_idx
  ON external_wiki_user (eu_local_id)
...
CREATE TABLE ipblocks (
  ...
  ipb_wiki_user              BIGINT NOT NULL DEFAULT 0,
  --           REFERENCES wiki_user(wiki_user_id) ON DELETE SET NULL,
...
CREATE INDEX ipb_wiki_user 
  ON ipblocks (ipb_wiki_user);
...
CREATE TABLE image (
  ...
  img_wiki_user         BIGINT NOT NULL DEFAULT 0,
  --         REFERENCES wiki_user(wiki_user_id) ON DELETE SET NULL,
...
CREATE TABLE oldimage (
  ...
  oi_wiki_user          BIGINT NOT NULL DEFAULT 0,
  --            REFERENCES wiki_user(wiki_user_id) ON DELETE SET NULL,
  oi_wiki_user_text     VARCHAR(255) NOT NULL,
...
CREATE TABLE filearchive (
  ...
  fa_deleted_wiki_user       BIGINT NOT NULL DEFAULT 0,
  --            REFERENCES wiki_user(wiki_user_id) ON DELETE SET NULL,
  ...
  fa_wiki_user               BIGINT NOT NULL DEFAULT 0,
  --            REFERENCES wiki_user(wiki_user_id) ON DELETE SET NULL,
  fa_wiki_user_text          VARCHAR(255) NOT NULL,
...
CREATE INDEX fa_nowiki_user
  ON filearchive (fa_deleted_wiki_user);
...
CREATE TABLE recentchanges (
  ...
  rc_wiki_user            BIGINT NOT NULL DEFAULT 0,
  --        REFERENCES wiki_user(wiki_user_id) ON DELETE SET NULL,
  rc_wiki_user_text       VARCHAR(255) NOT NULL,
...
CREATE TABLE watchlist (
  wl_wiki_user                   BIGINT NOT NULL DEFAULT 0,
  --  REFERENCES wiki_user(wiki_user_id) ON DELETE CASCADE,
  wl_namespace              SMALLINT NOT NULL DEFAULT 0,
  wl_title                  VARCHAR(255) NOT NULL,
  wl_notificationtimestamp  TIMESTAMP(3)
);
CREATE UNIQUE INDEX wl_wiki_user_namespace_title
  ON watchlist (wl_namespace, wl_title, wl_wiki_user);
...
CREATE INDEX logging_wiki_user_time
  ON logging (log_timestamp, log_wiki_user);
CREATE INDEX logging_page_time
  ON logging (log_namespace, log_title, log_timestamp);
CREATE INDEX log_wiki_user_type_time
  ON logging (log_wiki_user, log_type, log_timestamp);
...
CREATE TABLE protected_titles (
  ...
  pt_wiki_user        BIGINT NOT NULL DEFAULT 0,
  --       REFERENCES wiki_user(wiki_user_id) ON DELETE SET NULL,
...
CREATE UNIQUE INDEX wiki_user_properties_wiki_user_property
  ON wiki_user_properties (up_wiki_user, up_property);
CREATE INDEX wiki_user_properties_property
  ON wiki_user_properties (up_property);
...
CREATE INDEX us_wiki_user
  ON uploadstash (us_wiki_user);
...
CREATE TABLE wiki_user_former_groups (
  ufg_wiki_user   BIGINT NOT NULL DEFAULT 0,
  ufg_group  VARCHAR(16) FOR BIT DATA NOT NULL
);
CREATE UNIQUE INDEX ufg_wiki_user_group
  ON wiki_user_former_groups (ufg_wiki_user, ufg_group);
