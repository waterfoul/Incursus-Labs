 ...
 * and to find new page edits by wiki_users.
 *
...
class PopulateParentId extends LoggedUpdateMaintenance {
	...
	protected function doDBUpdates() {
		...
		$start = ->selectField( 'revision', 'MIN(rev_id)', false, __FUNCTION__ );
		$end = ->selectField( 'revision', 'MAX(rev_id)', false, __FUNCTION__ );
		...
		while ( $blockStart <= $end ) {
			...
			foreach ( $res as $row ) {
				...
				$previousID = ->selectField( 'revision', 'rev_id',
				...
				if ( !$previousID ) {
					...
					$lastTimestamp = ->selectField( 'revision', 'rev_timestamp',
						array( 'rev_page' => $row->rev_page, "rev_timestamp < " . ->addQuotes( $row->rev_timestamp ) ),
						__METHOD__,
