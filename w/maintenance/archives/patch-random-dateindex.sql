...
-- cur and old need (wiki_user,timestamp) index for contribs
-- cur and old need (wiki_user_text,timestamp) index for contribs

ALTER TABLE /*$wgDBprefix*/cur
  DROP INDEX cur_wiki_user,
  DROP INDEX cur_wiki_user_text,
  ...
  ADD INDEX wiki_user_timestamp (cur_wiki_user,inverse_timestamp),
  ADD INDEX wiki_usertext_timestamp (cur_wiki_user_text,inverse_timestamp);
...
ALTER TABLE /*$wgDBprefix*/old
  DROP INDEX old_wiki_user,
  DROP INDEX old_wiki_user_text,
  ADD COLUMN inverse_timestamp char(14) binary NOT NULL default '',
  ADD INDEX name_title_timestamp (old_namespace,old_title,inverse_timestamp),
  ADD INDEX wiki_user_timestamp (old_wiki_user,inverse_timestamp),
  ADD INDEX wiki_usertext_timestamp (old_wiki_user_text,inverse_timestamp);
