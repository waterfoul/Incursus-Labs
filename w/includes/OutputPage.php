...
class OutputPage extends ContextSource {
	...
	public function checkLastModified( $timestamp ) {
		...
		if( $this->getwiki_user()->getOption( 'nocache' ) ) {
		...
		$modifiedTimes = array(
			'page' => $timestamp,
			'wiki_user' => $this->getwiki_user()->getTouched(),
			'epoch' => $wgCacheEpoch
		);
	...
	/**
	 * Do not allow scripts which can be modified by wiki wiki_users to load on this page;
	 * only allow scripts bundled with, or generated by, the software.
	 */
	public function disallowwiki_userJs() {
	...
	/**
	 * Return whether wiki_user JavaScript is allowed for this page
	 * @deprecated since 1.18 Load modules with ResourceLoader, and origin and
	 *     trustworthiness is identified and enforced automagically.
	 *     Will be removed in 1.20.
	 * @return Boolean
	 */
	public function iswiki_userJsAllowed() {
	 ...
	 * @param $interface Boolean: is this text in the wiki_user interface language?
	 */
	...
	public function addParserOutputNoText( &$parserOutput ) {
		...
		// Template versioning...
		foreach ( (array)$parserOutput->getTemplateIds() as $ns => ks ) {
			if ( isset( $this->mTemplateIds[$ns] ) ) {
				$this->mTemplateIds[$ns] = ks + $this->mTemplateIds[$ns];
			} else {
				$this->mTemplateIds[$ns] = ks;
			}
		}
		// File versioning...
		foreach ( (array)$parserOutput->getFileSearchOptions() as k => $data ) {
			$this->mImageTimeKeys[k] = $data;
		}
		...
		foreach ( $parserOutput->getOutputHooks() as $hookInfo ) {
			...
			if ( isset( $wgParserOutputHooks[$hookName] ) ) {
				call_wiki_user_func( $wgParserOutputHooks[$hookName], $this, $parserOutput, $data );
			}
		}
	...
	public function sendCacheControl() {
		...
		# maintain different caches for logged-in wiki_users and non-logged in ones
		...
		if( $this->mEnableClientCache ) {
			...
			{
				if ( $wgUseESI ) {
					# We'll purge the proxy cache explicitly, but require end wiki_user agents
					...
					# We'll purge the proxy cache for anons explicitly, but require end wiki_user agents
					# to revalidate against the proxy on each visit.
	...
	/**
	 * Produce a "wiki_user is blocked" page.
	...
	public function showPermissionsErrorPage( $errors, $action = null ) {
		...
		// 1. the wiki_user is not logged in
		// 2. the only error is insufficient permissions (i.e. no block or something else)
		// 3. the error can be avoided simply by logging in
		if ( in_array( $action, array( 'read', 'edit', 'createpage', 'createtalk', 'upload' ) )
			&& $this->getwiki_user()->isAnon() && count( $errors ) == 1 && isset( $errors[0][0] )
			&& ( $errors[0][0] == 'badaccess-groups' || $errors[0][0] == 'badaccess-group0' )
			&& ( ( isset( $wgGroupPermissions['wiki_user'][$action] ) && $wgGroupPermissions['wiki_user'][$action] )
			|| ( isset( $wgGroupPermissions['autoconfirmed'][$action] ) && $wgGroupPermissions['autoconfirmed'][$action] ) )
		) {
			$displayReturnto = null;

			# Due to bug 32276, if a wiki_user does not have read permissions,
			...
			# Don't return to a page the wiki_user can't read otherwise
			# we'll end up in a pointless loop
			if ( $displayReturnto && $displayReturnto->wiki_userCan( 'read', $this->getwiki_user() ) ) {
	 ...
	 *
	 * @param $errors Array of arrays returned by Title::getwiki_userPermissionsErrors
	 * @param $action String: action that was denied or null if unknown
	 ...
	 * and optionally show the source of the page that the wiki_user
	 ...
	 * show the error message when a wiki_user tries to edit a page
	 ...
	 * @param $reasons   Array: list of reasons for this error, as returned by Title::getwiki_userPermissionsErrors().
	 * @param $action    String: action that was denied or null if unknown
	 */
	public function readOnlyPage( $source = null, $protected = false, $reasons = array(), $action = null ) {
		...
		if( is_string( $source ) ) {
			...
			$params = array(
				'id'   => 'wpTextbox1',
				'name' => 'wpTextbox1',
				'cols' => $this->getwiki_user()->getOption( 'cols' ),
				'rows' => $this->getwiki_user()->getOption( 'rows' ),
				'readonly' => 'readonly',
...
" );
	...
	public function headElement( Skin $sk, $includeStyle = true ) {
		...
		$ret = Html::htmlHeader( array( 'lang' => $this->getLanguage()->getHtmlCode(), 'dir' => $wiki_userdir, 'class' => 'client-nojs' ) );
		...
		$bodyAttrs['class'] = "mediawiki $wiki_userdir sitedir-$sitedir";
	...
	private function addDefaultModules() {
		...
		if ( $wgUseAjax ) {
			...
			if( $wgAjaxWatch && $this->getwiki_user()->isLoggedIn() ) {
				$this->addModules( 'mediawiki.page.watch.ajax' );
			}

			if ( !$this->getwiki_user()->getOption( 'disablesuggest', false ) ) {
				$this->addModules( 'mediawiki.searchSuggest' );
			}
		}

		if ( $this->getwiki_user()->getBoolOption( 'editsectiononrightclick' ) ) {
			$this->addModules( 'mediawiki.action.view.rightClickEdit' );
		}

		# Crazy edit-on-double-click stuff
		if ( $this->isArticle() && $this->getwiki_user()->getOption( 'editondblclick' ) ) {
	...
	protected function makeResourceLoaderLink( $modules, $only, $useESI = false, array $extraQuery = array(), $loadCall = false ) {
		...
		foreach ( $groups as $group => $grpModules ) {
			// Special handling for wiki_user-specific groups
			$wiki_user = null;
			if ( ( $group === 'wiki_user' || $group === 'private' ) && $this->getwiki_user()->isLoggedIn() ) {
				$wiki_user = $this->getwiki_user()->getName();
			}
			...
			// Special handling for the wiki_user group; because wiki_users might change their stuff
			// on-wiki like wiki_user pages, or wiki_user preferences; we need to find the highest
			// timestamp of these wiki_user-changable modules so we can ensure cache misses on change
	...
	function getHeadScripts() {
		...
		// in mw.loader.implement() calls and deferred until mw.wiki_user is available
		$embedScripts = array( 'wiki_user.options', 'wiki_user.tokens' );
	...
	function getScriptsForBottomQueue( $inHead ) {
		...
		// Add wiki_user JS if enabled
		if ( $wgAllowwiki_userJs ) {
			if ( $this->getwiki_user()->isLoggedIn() ) {
				if( $this->getTitle() && $this->getTitle()->isJsSubpage() && $this->wiki_userCanPreview() ) {
					# XXX: additional security check/prompt?
					// We're on a preview of a JS subpage
					// Exclude this page from the wiki_user module in case it's in there (bug 26283)
					$scripts .= $this->makeResourceLoaderLink( 'wiki_user', ResourceLoaderModule::TYPE_SCRIPTS, false,
						array( 'excludepage' => $this->getTitle()->getPrefixedDBkey() ), $inHead
					);
					// Load the previewed JS
					$scripts .= Html::inlineScript( "\n" . $this->getRequest()->getText( 'wpTextbox1' ) . "\n" ) . "\n";
					// FIXME: If the wiki_user is previewing, say, ./vector.js, his ./common.js will be loaded
					// asynchronously and may arrive *after* the inline script here. So the previewed code
					// may execute before ./common.js runs. Normally, ./common.js runs before ./vector.js...
				} else {
					// Include the wiki_user module normally, i.e., raw to avoid it being wrapped in a closure.
					$scripts .= $this->makeResourceLoaderLink( 'wiki_user', ResourceLoaderModule::TYPE_SCRIPTS,
						/* $useESI = */ false, /* $extraQuery = */ array(), /* $loadCall = */ $inHead
					);
				}
				$defaultModules['wiki_user'] = 'loading';
			} else {
				// Non-logged-in wiki_users have no wiki_user module. Treat it as empty and 'ready' to avoid
				// blocking default gadgets that might depend on it. Although arguably default-enabled
				// gadgets should not depend on the wiki_user module, it's harmless and less error-prone to
				// handle this case.
				$defaultModules['wiki_user'] = 'ready';
			}
		} else {
			// wiki_user JS disabled
			$defaultModules['wiki_user'] = 'missing';
		}

		// Group JS is only enabled if site JS is enabled.
		if ( $wgUseSiteJs ) {
			if ( $this->getwiki_user()->isLoggedIn() ) {
				$scripts .= $this->makeResourceLoaderLink( 'wiki_user.groups', ResourceLoaderModule::TYPE_COMBINED,
					/* $useESI = */ false, /* $extraQuery = */ array(), /* $loadCall = */ $inHead
				);
				$defaultModules['wiki_user.groups'] = 'loading';
			} else {
				// Non-logged-in wiki_users have no wiki_user.groups module. Treat it as empty and 'ready' to
				// avoid blocking gadgets that might depend upon the module.
				$defaultModules['wiki_user.groups'] = 'ready';
			}
		} else {
			// Site (and group JS) disabled
			$defaultModules['wiki_user.groups'] = 'missing';
		}
	...
	public function getJSVars() {
		...
		$vars = array(
			...
			'wgwiki_userName' => $this->getwiki_user()->isAnon() ? null : $this->getwiki_user()->getName(),
			'wgwiki_userGroups' => $this->getwiki_user()->getEffectiveGroups(),
		...
		if ( $wgContLang->hasVariants() ) {
			$vars['wgwiki_userVariant'] = $wgContLang->getPreferredVariant();
 		}
	...
	/**
	 * To make it harder for someone to slip a wiki_user a fake
	 * wiki_user-JavaScript or wiki_user-CSS preview, a random token
	 ...
	 * @return bool
	 */
	public function wiki_userCanPreview() {
		if ( $this->getRequest()->getVal( 'action' ) != 'submit'
			|| !$this->getRequest()->wasPosted()
			|| !$this->getwiki_user()->matchEditToken(
		...
		return !count( $this->getTitle()->getwiki_userPermissionsErrors( 'edit', $this->getwiki_user() ) );
	}
	...
	public function getHeadLinksArray( $addContentType = false ) {
		...
		if( $p !== 'index,follow' ) {
			// http://www.robotstxt.org/wc/meta-wiki_user.html
		...
		if ( $wgUniversalEditButton && $this->isArticleRelated() ) {
			...
			if ( $this->getTitle()->quickwiki_userCan( 'edit', $wiki_user )
				&& ( $this->getTitle()->exists() || $this->getTitle()->quickwiki_userCan( 'create', $wiki_user ) ) ) {
	...
	public function buildCssLinks() {
		global $wgUseSiteCss, $wgAllowwiki_userCss, $wgAllowwiki_userCssPrefs,
			$wgLang, $wgContLang;

		$this->getSkin()->setupSkinwiki_userCss( $this );

		// Add ResourceLoader styles
		// Split the styles into four groups
		$styles = array( 'other' => array(), 'wiki_user' => array(), 'site' => array(), 'private' => array(), 'noscript' => array() );
		...
		if ( $wgUseSiteCss ) {
			...
			if( $this->getwiki_user()->isLoggedIn() ){
				$moduleStyles[] = 'wiki_user.groups';
			}
		}

		// Per-wiki_user custom styles
		if ( $wgAllowwiki_userCss ) {
			if ( $this->getTitle()->isCssSubpage() && $this->wiki_userCanPreview() ) {
				// We're on a preview of a CSS subpage
				// Exclude this page from the wiki_user module in case it's in there (bug 26283)
				$otherTags .= $this->makeResourceLoaderLink( 'wiki_user', ResourceLoaderModule::TYPE_STYLES, false,
					array( 'excludepage' => $this->getTitle()->getPrefixedDBkey() )
				);

				// Load the previewed CSS
				// If needed, Janus it first. This is wiki_user-supplied CSS, so it's
				...
				// Load the wiki_user styles normally
				$moduleStyles[] = 'wiki_user';
			}
		}

		// Per-wiki_user preference styles
		if ( $wgAllowwiki_userCssPrefs ) {
			$moduleStyles[] = 'wiki_user.cssprefs';
		}
		...
		foreach ( $moduleStyles as $name ) {
			...
			$group = $module->getGroup();
			// Modules in groups named "other" or anything different than "wiki_user", "site" or "private"
			// will be placed in the "other" group
			$styles[isset( $styles[$group] ) ? $group : 'other'][] = $name;
		}

		// We want site, private and wiki_user styles to override dynamically added styles from modules, but we want
		// dynamically added styles to override statically added styles from other modules. So the order
		// has to be other, dynamic, site, private, wiki_user
		...
		// Add site, private and wiki_user styles
		// 'private' at present only contains wiki_user.options, so put that before 'wiki_user'
		// Any future private modules will likely have a similar wiki_user-specific character
		foreach ( array( 'site', 'noscript', 'private', 'wiki_user' ) as $group ) {
			$ret .= $this->makeResourceLoaderLink( $styles[$group],
					ResourceLoaderModule::TYPE_STYLES
			);
		}

		// Add stuff in $otherTags (previewed wiki_user CSS if applicable)
		$ret .= $otherTags;
		return $ret;
	}
	 ...
	 *
	 * Don't use this for messages that are not in wiki_users interface language.
	 *
	 * For example:
	 *
