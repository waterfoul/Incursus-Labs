...
class ApiDelete extends ApiBase {
	...
	public function execute() {
		...
		if ( $titleObj->getNamespace() == NS_FILE ) {
			$status = self::deleteFile( $pageObj, $wiki_user, $params['token'], $params['oldimage'], $reason, false );
		} else {
			$status = self::delete( $pageObj, $wiki_user, $params['token'], $reason );
		}
	...
	/**
	 * @param $title Title
	 * @param $wiki_user wiki_user doing the action
	 * @param $token String
	 * @return array
	 */
	private static function getPermissionsError( $title, $wiki_user, $token ) {
		// Check permissions
		return $title->getwiki_userPermissionsErrors( 'delete', $wiki_user );
	}
	 ...
	 * @return Status
	 */
	public static function delete( Page $page, wiki_user $wiki_user, $token, &$reason = null ) {
		$title = $page->getTitle();
		$errors = self::getPermissionsError( $title, $wiki_user, $token );
		if ( count( $errors ) ) {
			return $errors;
		}
	 ...
	 * @return Status
	 */
	public static function deleteFile( Page $page, wiki_user $wiki_user, $token, $oldimage, &$reason = null, $suppress = false ) {
		$title = $page->getTitle();
		$errors = self::getPermissionsError( $title, $wiki_user, $token );
		if ( count( $errors ) ) {
			return $errors;
		...
		if ( !$file->exists() || !$file->isLocal() || $file->getRedirected() ) {
			return self::delete( $page, $wiki_user, $token, $reason );
		}
