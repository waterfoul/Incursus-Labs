...
class BlockTest extends MediaWikiLangTestCase {
	...
	function addDBData() {
		...
		$wiki_user = wiki_user::newFromName( 'UTBlockee' );
		if( $wiki_user->getID() == 0 ) {
			$wiki_user->addToDatabase();
			$wiki_user->setPassword( 'UTBlockeePassword' );
		...
		$this->block = new Block( 'UTBlockee', $wiki_user->getID(), 0,
	...
	function testBug29116LoadWithEmptyIp( $vagueTarget ) {
		...
		$this->assertTrue( ($uid > 0), 'Must be able to look up the target wiki_user during tests' );

		$block = new Block();
		$ok = $block->load( $vagueTarget, $uid );
		$this->assertTrue( $ok, "Block->load() with empty IP and wiki_user ID '$uid' should return a block" );
	...
	function testBlockedwiki_userCanNotCreateAccount() {
		$wiki_username = 'Blockedwiki_userToCreateAccountWith';
		$u = wiki_user::newFromName( $wiki_username );
		...
		// Sanity check
		$this->assertNull(
			Block::newFromTarget( $wiki_username ),
			"$wiki_username should not be blocked"
		);
		...
		$this->assertFalse(
			...
			"Our sandbox wiki_user should be able to create account before being blocked"
		);

		// Foreign perspective (blockee not on current wiki)...
		$block = new Block(
			/* $address */ $wiki_username,
			/* $wiki_user */ 14146,
			/* $by */ 0,
		...
		$wiki_userBlock = Block::newFromTarget( $wiki_username );
		$this->assertTrue(
		...
		$this->assertInstanceOf(
			'Block',
			$wiki_userBlock,
			"'$wiki_username' block block object should be existent"
		);
		...
		$this->assertTrue(
			...
			"Our sandbox wiki_user '$wiki_username' should NOT be able to create account"
		);
	}
	...
	function testCrappyCrossWikiBlocks() {
		...
		$wiki_user = wiki_user::newFromName( 'wiki_userOnForeignWiki' );
		$wiki_user->addToDatabase();
		// Set wiki_user ID to match the test value
		$this->db->update( 'wiki_user', array( 'wiki_user_id' => 14146 ), array( 'wiki_user_id' => $wiki_user->getId() ) );
		$wiki_user = null; // clear
